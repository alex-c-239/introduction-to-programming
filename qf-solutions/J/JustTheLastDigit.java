...
public class JustTheLastDigit {
    ...
    public static void main(String[] args) {
        ...
        int[][] numberOfPaths = new int[n][n];

        for (int i = 0; i < n; ++i) {
            String s = scanner.nextNumber();
            for (int j = 0; j < n; ++j) {
                numberOfPaths[i][j] = s.charAt(j) - '0';
            }
        }

        for (int i = 0; i < n; ++i) {
            for(int j = 0; j < n; ++j) {
                if (numberOfPaths[i][j] != 1) {
                    System.out.print(0);
                    continue;
                }
                ...
                for (int k = j + 1; k < n; ++k) {
                    numberOfPaths[i][k] -= numberOfPaths[j][k];
                    while (numberOfPaths[i][k] < 0) {
                        numberOfPaths[i][k] += 10;
                    }
                }
            }
            System.out.println();
